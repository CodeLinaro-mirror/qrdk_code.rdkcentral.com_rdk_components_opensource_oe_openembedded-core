...
DEPLOY_DIR_SPDX ??= "${TMPDIR}/deploy/spdx/${MACHINE}"
...
python do_create_spdx() {
    ...
    if not recipe_spdx_is_native(d, recipe):
        ...
        for package in d.getVar("PACKAGES").split():
            ...
            pkg_name = d.getVar("PKG_%s" % package) or d.getVar("PKG:%s" % package) or package
...
addtask do_create_spdx after do_package do_packagedata do_package_write_ipk do_unpack before do_populate_sdk do_build do_rm_work
...
python do_create_runtime_spdx() {
    ...
    if not is_native:
        ...
        for package in d.getVar("PACKAGES").split():
            ...
            pkg_data = oe.packagedata.read_subpkgdata_dict(pkg_name, d)
            if len(pkg_data) == 0:
                pkg = pkg_name
            else:
                pkg = pkg_data["PKG"]
            ...
            pkg_spdx_path = deploy_dir_spdx / "packages" / (pkg + ".spdx.json")
            ...
            for p in package_doc.packages:
                ...
                bb.fatal("Package '%s' not found in %s" % (pkg, pkg_spdx_path))

            runtime_doc = oe.spdx.SPDXDocument()
            runtime_doc.name = "runtime-" + pkg
...
POPULATE_SDK_POST_HOST_COMMAND_task-populate-sdk += " sdk_host_combine_spdx; "
POPULATE_SDK_POST_TARGET_COMMAND_task-populate-sdk += " sdk_target_combine_spdx; "
...
def combine_spdx(d, rootfs_name, rootfs_deploydir, rootfs_spdxid, packages):
    ...
    bb.warn(f'spdx tar path {spdx_tar_path}')
